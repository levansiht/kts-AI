...
const FullscreenModal: React.FC<{ imageUrl: string; onClose: () => void }> = ({
  imageUrl,
  onClose,
}) => (
  <div
...
export const VirtualTourTab: React.FC<VirtualTourTabProps> = ({
  setActiveTab,
  setImageForEditing,
  ...
  const handleNavigate = useCallback(
    async (moveType: TourMoveType) => {
      ...
      try {
        const newImageSrc = await generateVirtualTourImage(
          currentImage,
          moveType,
          magnitude
        );
        if (newImageSrc) {
          const newImage: SourceImage = {
            ...
            mimeType:
              newImageSrc.match(/data:(image\/[a-z]+);/)?.[1] || "image/png",
            dataUrl: newImageSrc,
            name: `virtual_tour_${Date.now()}.png`,
          };
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex items-center gap-2 bg-[var(--bg-surface-3)] rounded-md p-1">
                    {(["15", "30", "45"] as const).map((angle) => (
                      <button
                        key={angle}
                        onClick={() =>
                          setMagnitude(Number(angle) as 15 | 30 | 45)
                        }
                        className={`w-full text-sm font-semibold py-1.5 rounded-md transition-colors ${
                          magnitude === Number(angle)
                            ? "bg-[var(--bg-interactive)] text-[var(--text-interactive)] shadow"
                  ...
                  <p className="text-xs text-[var(--text-tertiary)] mt-1">
                    Áp dụng cho Pan và Orbit. Zoom có mức độ cố định.
                  </p>
                </div>
                <div className="space-y-2">
                  <h3 className="font-semibold text-center text-[var(--text-secondary)]">
                    Pan (Xoay camera tại chỗ)
                  </h3>
                  <div className="grid grid-cols-3 gap-2 p-2 bg-[var(--bg-surface-4)]/50 rounded-lg">
                    ...
                    <ControlButton
                      ...
                      label="Trái"
                      onClick={() => handleNavigate("pan-left")}
                      disabled={isLoading}
                      title="Xoay trái"
                    />
                    ...
                    <ControlButton
                      ...
                      label="Phải"
                      onClick={() => handleNavigate("pan-right")}
                      disabled={isLoading}
                      title="Xoay phải"
                    />
                    <div />
                    <ControlButton
                      ...
                      label="Xuống"
                      onClick={() => handleNavigate("pan-down")}
                      disabled={isLoading}
                      title="Nghiêng xuống"
                    />
                    <div />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 p-2 bg-[var(--bg-surface-4)]/50 rounded-lg">
                      <ControlButton
                        ...
                        label="Quay Trái"
                        onClick={() => handleNavigate("orbit-left")}
                        disabled={isLoading}
                        title="Đi vòng sang trái"
                      />
                      <ControlButton
                        icon="arrow-uturn-right"
                        label="Quay Phải"
                        onClick={() => handleNavigate("orbit-right")}
                        disabled={isLoading}
                        title="Đi vòng sang phải"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 p-2 bg-[var(--bg-surface-4)]/50 rounded-lg">
                      <ControlButton
                        ...
                        label="Gần Lại"
                        onClick={() => handleNavigate("zoom-in")}
                        disabled={isLoading}
                        title="Phóng to"
                      />
                      <ControlButton
                        ...
                        label="Ra Xa"
                        onClick={() => handleNavigate("zoom-out")}
                        disabled={isLoading}
                        title="Thu nhỏ"
                      />
                    </div>
            ...
            <div className="w-full aspect-video bg-black/20 rounded-lg flex items-center justify-center min-h-[400px] lg:min-h-[550px] relative overflow-hidden group">
              {isLoading && (
                <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center z-20">
                  ...
                  <p className="mt-4 font-semibold text-sm text-slate-200">
                    {loadingMessage}
                  </p>
                </div>
              )}
