      ...
      >
        {sourceImage ? (
          ...
          <div className="text-center text-[var(--text-secondary)] pointer-events-none">
            <Icon name="photo" className="w-12 h-12 mx-auto mb-2 text-[var(--text-tertiary)]" />
...
export const UpscaleTab: React.FC = () => {
  ...
  const handleUpscale = async () => {
    ...
    try {
      ...
      alert(`Đã xảy ra lỗi khi upscale: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 flex flex-col gap-8">
          <Section title="1. Ảnh Cần Upscale">
            <ImageUpload sourceImage={sourceImage} onImageUpload={(img) => { setSourceImage(img); setResultImage(null); }} onRemove={() => { setSourceImage(null); setResultImage(null); }} />
          </Section>
          {sourceImage && (
            <Section title="2. Tùy Chọn Độ Phân Giải">
              <div className="space-y-4">
                <div className="flex gap-4">
                  <button onClick={() => setTargetResolution("2k")} className={`flex-1 py-4 px-2 rounded-lg border-2 font-bold text-center transition-all ${targetResolution === "2k" ? "border-[var(--border-accent)] bg-[var(--bg-surface-3)] text-[var(--text-accent)] shadow-md" : "border-[var(--border-2)] bg-transparent text-[var(--text-secondary)] hover:border-[var(--text-secondary)]"}`}>
                    2K Resolution
                  </button>
                  <button onClick={() => setTargetResolution("4k")} className={`flex-1 py-4 px-2 rounded-lg border-2 font-bold text-center transition-all ${targetResolution === "4k" ? "border-[var(--border-accent)] bg-[var(--bg-surface-3)] text-[var(--text-accent)] shadow-md" : "border-[var(--border-2)] bg-transparent text-[var(--text-secondary)] hover:border-[var(--text-secondary)]"}`}>
                    4K Resolution
                  </button>
                </div>
                <div className="p-3 bg-[var(--bg-surface-2)] rounded-md text-xs text-[var(--text-secondary)]">
                  <p className="font-semibold mb-1 flex items-center gap-1">
                    <Icon name="sparkles" className="w-3 h-3" /> Sử dụng mô hình Nano Banana Pro
                  </p>
                  <p>Mô hình chuyên dụng cho việc tái tạo chi tiết và nâng cao chất lượng hình ảnh sắc nét.</p>
                </div>
                <button onClick={handleUpscale} disabled={isLoading} className="w-full bg-[var(--bg-interactive)] hover:bg-[var(--bg-interactive-hover)] text-[var(--text-interactive)] font-bold py-4 px-4 rounded-lg transition-all flex items-center justify-center gap-2 shadow-lg disabled:bg-[var(--bg-disabled)] disabled:cursor-not-allowed transform hover:scale-[1.02]">
                  {isLoading ? (
                    <>
                    ...
                    <>
                      ...
                      <span>Bắt Đầu Upscale {targetResolution.toUpperCase()}</span>
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="w-full aspect-video bg-black/20 rounded-lg flex items-center justify-center min-h-[500px] relative group">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-4 font-semibold text-lg text-[var(--text-primary)]">AI đang xử lý ảnh...</p>
                  <p className="text-sm text-[var(--text-secondary)] mt-2">Quá trình này có thể mất vài giây đến một phút.</p>
                </div>
              ) : resultImage ? (
                <>
                  <img src={resultImage} alt="Upscaled result" className="max-w-full max-h-full object-contain rounded-md shadow-2xl" />
                  <div className="absolute top-4 right-4 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                    <button onClick={() => setIsResultFullscreen(true)} className="bg-[var(--bg-surface-3)]/90 backdrop-blur-md border border-[var(--border-2)] hover:bg-[var(--bg-interactive)] text-[var(--text-primary)] hover:text-[var(--text-interactive)] font-bold text-sm px-4 py-3 rounded-lg transition-colors flex items-center gap-2 shadow-lg" title="Xem Toàn Màn Hình">
                      <Icon name="arrows-expand" className="w-5 h-5" />
                      <span>Phóng To</span>
                    </button>
                    <a href={resultImage} download={`nbox-ai-upscale-${targetResolution}-${Date.now()}.png`} className="bg-[var(--bg-surface-3)]/90 backdrop-blur-md border border-[var(--border-2)] hover:bg-[var(--bg-interactive)] text-[var(--text-primary)] hover:text-[var(--text-interactive)] font-bold text-sm px-4 py-3 rounded-lg transition-colors flex items-center gap-2 shadow-lg" aria-label="Tải ảnh" title="Tải ảnh">
                ...
                <div className="text-center text-[var(--text-tertiary)]">
                  <Icon name="arrow-up-circle" className="w-20 h-20 mx-auto mb-6 opacity-50" />
                  <p className="text-lg font-medium">Kết quả Upscale sẽ xuất hiện tại đây.</p>
                </div>
              )}
            </div>
          </Section>
